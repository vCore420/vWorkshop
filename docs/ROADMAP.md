...
## Version 4, Phase 2a — Playtesting Notes, Continued (v4.0.2a)

**Goal:** the first of five waves carrying forward Vi's own post-Version-3
field notes, per `docs/ROADMAP_V4.md`'s own Phase 2 — a grab-bag by
nature, resisting a unifying narrative on purpose. Wave A: four small,
independent, unambiguous fixes with no open design questions, investigated
via a parallel research pass (alongside Waves B and C's own investigation)
before any code changed.

**Outdoor bench facing the wrong way** — `WorkshopRoom.js`'s own backrest
offset (`benchZ + 0.17`) put the backrest *further* from the wall than the
seat, an inverted sign rather than a deeper geometry problem. Fixed to
the ordinary arrangement (backrest near the wall, open seating edge
toward the yard).

**Computer chair castors, still not turning with their own arm despite
Phase 14's own fix** — that fix (`castor.rotation.y = angle`) turned out
to be a complete no-op, not a partial one: re-deriving Three.js's actual
Euler composition formula showed the Y-rotation was being applied while
the castor cylinder was still perfectly symmetric about that exact axis,
so it changed nothing regardless of `angle`, for any of the five castors,
at any position. `castor.rotation.order = "YXZ"` reverses which rotation
runs first, letting `angle` actually reach the render — confirmed live by
reading back all five castors' own world-space orientation and finding
five genuinely distinct directions (evenly spaced, matching the base's
own five-fold symmetry) rather than one repeated five times.

**Wardrobe's own form overflowing its panel** — `<select>` elements inside
`.panel-row` never got the `min-width: 0` treatment `input[type="range"]`
already received for the identical bug class in an earlier phase; fixed
identically. Separately, Wardrobe never inherited the `.builder-workspace`
700px stacking breakpoint three other apps already share (deliberately —
it was kept on an older, separate class family so the two layouts
couldn't affect each other) — given a parallel breakpoint of its own,
same value, same technique, without merging the classes themselves.
Verified live at a genuine 375px mobile viewport: zero horizontal
overflow anywhere, real column stacking, the grid correctly collapsing to
one column.

**HUD compass clipping against the corner-controls row on touch** — the
existing global touch-comfort baseline (`@media (pointer: coarse) {
button { min-height: 40px; } }`) grows the corner-controls row taller
specifically on touch, pushing its bottom edge into the compass's own
fixed band — invisible on desktop, exactly matching "fine on desktop...
touch/mobile specifically." Fixed with a matching `@media (pointer:
coarse)` override on the compass's own `top` offset. This sandboxed
browser has no way to actually emulate `(pointer: coarse)` (confirmed
directly — `matchMedia` never reports `true` regardless of viewport size),
so verification was structural: the parsed CSS rule read back correctly
from the live stylesheet, and its arithmetic validated against the
corner-controls row's actual, currently-measured pixel height rather than
a simulated one. Noted in `.claude/DEV_NOTES.md` as a standing environment
limitation for future touch-only CSS work.

`docs/WORLD.md`, `docs/COMPUTER.md`, and `docs/RESPONSIVE.md` all updated
in place. Debug hooks stay in place through the remaining four waves of
this phase, removed only once Wave E closes.

## Version 4, Phase 2b — Playtesting Notes, Continued (v4.0.2b)

**Goal:** Wave B of five — "Phone wallpapers don't actually change when
picked, despite Phase 13's own wallpaper-customization work."
Investigated first, and it wasn't what it looked like: the data flow is
genuinely correct — the same shared `SettingsStore`, a real
`settings:changed` event, the correct `dataset.wallpaper` assignment on
the one persistent phone shell, correct higher-specificity CSS, no
competing hardcoded rule anywhere. Every link in that chain was traced
and confirmed working, independently of anything this wave touched.

**The actual bug was scope, not wiring.** `css/phone.css`'s
`[data-wallpaper]` tint rule was gated behind
`.workshop-phone-content:has(.workshop-phone-home-grid)`, which only ever
matches while the Home screen specifically is mounted — but the wallpaper
picker lives in the Settings app, a different screen entirely. A player
choosing a wallpaper saw only a button's own active-state highlight;
the tint itself only appeared after backing all the way out to Home, and
even there it's a subtle ~16–20% blend easy to miss. `docs/PHONE.md`'s
own account of the mechanism was accurate throughout — the docs weren't
lying, they just never surfaced that the mechanism's visible effect was
scoped somewhere a player wouldn't be looking.

**The fix:** drop the `:has()` gate, so the tint becomes the phone's own
backdrop on every screen — the same way a real device's wallpaper isn't
confined to its home screen — rather than redesigning the Settings
picker itself (that's `docs/ROADMAP_V4.md`'s own separate Phase 3, which
explicitly depends on this exact bug being fixed first). The base
`.workshop-phone-content` rule's own now-fully-shadowed `background:
var(--paper)` declaration was removed alongside it, rather than left
sitting underneath a rule that always overrides it.

**Verified live**, not just read as correct: opened the real, running
`PhoneSystem`, navigated to the real Settings screen, and read
`getComputedStyle(...).backgroundImage` on the real DOM before and after
clicking a real wallpaper preset button — confirmed `"none"` on a stale
tab (a genuine CSS-caching gotcha hit and resolved mid-verification, now
recorded in `.claude/DEV_NOTES.md` alongside this project's existing
module-staleness notes: a `<link>` stylesheet doesn't refetch just
because the file changed on disk, the same "prefer a fresh tab" fix
already established for stale ES modules applies here too) becomes a
real, correctly-tinted gradient on a fresh tab, without ever navigating
away from Settings. `docs/PHONE.md` updated in place.

## Version 4, Phase 2c — Playtesting Notes, Continued (v4.0.2c)

**Goal:** Wave C of five — "the architectural front door still doesn't
hinge correctly, even after Phase 14 Wave 2's own attempt: its highest
points visibly swing outward (south), away from the house, as it opens."

**Phase 14's own account was accurate about what it did — the pivot
genuinely does sit at the wall's true outer face.** What it didn't
account for was the *mesh*: that fix deliberately kept the panel offset
by the full wall thickness (0.3m) from the pivot, specifically so the
closed door's world position wouldn't visibly shift when the pivot moved.
That preserved the closed look at the cost of the open one — a door
leaf sitting 0.3m away from its own rotation axis doesn't swing like a
hinge, it orbits a point that isn't actually on it. Traced numerically
before touching any code: the hinge-side point's own world position at
closed vs. fully-open (the door's own 1.9 radian target angle) drifted
by roughly half a metre — reading exactly like "swings outward, away
from the house," not a clean hinge, and explaining precisely why the
symptom persisted despite a real, correctly-described earlier fix.

**The fix:** offset the mesh by half its own physical depth (0.03m)
instead of the wall's full thickness (0.3m), so the panel's hinge-side
edge lands exactly on the pivot — the same way a real door's hinge pin
sits right at the leaf's own edge, not floating inside the wall cavity
behind it. The closed door's world position does shift as a result (now
sitting near the outer face, inside the wall's own opening, rather than
flush with the interior face) — a genuine, correct consequence: a leaf
actually hinged at the outer face has to live near it, not the far side
of the wall, for "hinge" to mean anything geometrically.

**Verified live** with the identical numeric method the investigation
itself used, re-run against the fixed code rather than trusted on sight:
read the hinge-side corner's own world position at closed and fully-open
for both panels directly from the running scene graph
(`RoomLayoutSystem.room.doorPanels.left`/`.right`) — 0.0000m drift for
both, down from the ~0.5m measured before. `docs/WORLD.md` updated in
place, in the same section documenting the front doors' own hinge
hardware.

## Version 4, Phase 2d — Playtesting Notes, Continued (v4.0.2d)

**Goal:** Wave D of five — "the Emote Wheel is getting better but still
off in design... something closer to FiveM's `qb-radialmenu` in feel,"
plus, separately, "the wheel should open on Tab, not G." Two genuine
product decisions resolved with Vi before implementation: a full
wedge-segment redesign (not a lighter refinement of the existing ring),
and confirmation the Tab rebind should proceed once a real conflict this
phase's own investigation found got resolved.

**The rebind needed a real fix first, not just a keybinding change.**
`EmoteWheelSystem.update()` had never checked anything about modal state
— apparently never a practical problem for G (rarely pressed while the
Computer/Phone/an overlay panel was open), but Tab is the exact key
those panels already use continuously to cycle focus between their own
controls (`focusTrap.js`, confirmed to never call `stopPropagation()`).
An unguarded rebind would have toggled the wheel on every focus-cycle
keypress inside any of them. The fix reuses `input.pointerLocked` — the
same signal every modal in this codebase already computes and relies on
(`exitPointerLock()`/`requestPointerLock()`, called by Computer, Phone,
Workbench, and every `OverlayManager` panel) — rather than inventing a
second way to ask the same question. `|| this.isOpen` in the same
condition lets Tab still close the wheel once it's open, even though
opening it releases pointer lock itself. Confirmed live, three ways:
Tab opens the wheel during simulated free-roam; Tab does nothing with
pointer lock simulated as released and the wheel closed (the modal
case); Tab still closes the wheel once open, with pointer lock released,
isolating that `|| this.isOpen` fallback specifically. Also swept: the
three literal "G" references this rebind would otherwise have left
stale (`EmoteWheelSystem.js`'s own docstring and hint text, `HUD.js`'s
corner-button label).

**The wedge redesign.** Rebuilt from circles-on-a-ring into genuine
pie-wedge segments — each gesture's own button now fills the entire ring
and is cut to its own slice with a JS-computed `clip-path:
polygon(...)`, sampling the arc every ~12° so a wide wedge (few total
gestures) still reads as a curved slice rather than a straight-edged
triangle. A real, independently clickable wedge-shaped hit area, not a
circular button on a decorative background. Icons are a small, honest
keyword heuristic against a clip's own name, falling back to a plain
generic glyph for anything player-authored that doesn't match — this
project ships no binary/image assets by design, so there was never a
real icon library to draw from instead. The centre hub grew larger and
became a genuine second close affordance; the bordered/backgrounded card
wrapper is gone for the ring itself, floating it directly over the game
view.

**Verified live**, structurally rather than by screenshot (per
`.claude/DEV_NOTES.md`): mounted and opened the real wheel, read every
wedge's own computed `clip-path` back, and confirmed each wedge's last
boundary point exactly matches the next wedge's first point all the way
around — the full circle tiled with zero gaps or overlaps, for a real
4-gesture wheel (Wave, Clap, Bow, Dance), each correctly matched to its
own icon by the keyword heuristic. `docs/PLAYER.md` updated in place.

## Version 4, Phase 2e — Playtesting Notes, Continued, closeout (v4.0.2e)

**Goal:** Wave E of five, and this phase's own closing wave — "Double
Door construction pieces still pivot both leaves together to one shared
corner, rather than opening independently like real French doors...
worth attempting now that it's been named twice." Resolved with Vi
before implementation: build the real fix now, not defer a third time.

**Investigation de-risked this substantially before any code changed.**
`docs/WORLDBUILDER.md`'s own prior account already named the correct
direction — "two separate placed objects, each with its own hinge" — but
read as a bigger architecture change than it turned out to be. The
already-real, already-correct single `Door` piece proved the actual
mechanism needed: `hingeOffset` (half its own width, hinging at its own
outer edge) via the existing, unmodified `DoorBehaviour`. No new
behaviour-system capability was needed — only two new Construction
Library pieces, **Door Leaf (Left)** and **Door Leaf (Right)**, each
sized to match one half of the existing combined `Double Door` piece and
carrying that identical mechanism, mirrored between the two.

**The existing `Double Door` piece is completely untouched** — any
existing save with one already placed keeps working exactly as it does
today; the two new pieces are a purely additive, more realistic
alternative alongside it, not a replacement.

**Verified live**, not just read as correct: spawned both new pieces
side by side in the real running engine, toggled each leaf's own real
`DoorBehaviour` interaction directly. Opening the left leaf left the
right leaf's position and rotation completely unchanged, and vice versa
— genuine independence, confirmed both directions. The left leaf's own
open→close round trip returned to its exact starting transform (no
drift). `docs/WORLDBUILDER.md` updated in place, including correcting
its own prior account of this as an unattempted direction.

**Phase close.** All five waves of "Playtesting Notes, Continued" are
now done: four small independent geometry/CSS fixes (Wave A), the phone
wallpaper visibility fix (Wave B), the front door hinge (Wave C), the
Emote Wheel's Tab rebind and wedge redesign (Wave D), and independent
double doors (Wave E) — matching Version 3 Phase 14's own five-wave
precedent for a grab-bag phase of this shape. Debug hooks removed from
`main.js`, `.claude/launch.json` reverted to port 8000. See
`docs/HISTORY.md` for the combined closing account.

## Version 4, Phase 3 — The Phone's Settings, Made Real (v4.0.3)

**Goal:** `docs/ROADMAP_V4.md`'s own Phase 3 — "I want this to look more
real and less basic drop down boxes and generic UI elements" for the
Phone's own Settings app. Depended on Phase 2's own wallpaper-visibility
fix (already shipped, v4.0.2b) and stayed explicitly scoped to the
Phone's own Settings screen; the PC Settings app's own Phone tab (a real
`<select>` dropdown, `SettingsApp.js`'s `renderPhone()`) was deliberately
left untouched — a player sets their device's own screen from the device
itself.

**Investigation found the real gap wasn't the picker, it was the theme.**
The existing wallpaper/border rows were already button-rows, not literal
`<select>` elements — but showed no colour at all, which is what "generic
UI elements" actually pointed at. There was no `brightness` or `theme`
field anywhere yet. Every app screen on the Phone hardcoded light-mode
colours directly — roughly fifteen rule blocks across `css/phone.css`.
**Decided with Vi:** dark theme goes phone-wide, not scoped to
Settings/Home only, so picking "Dark" changes what using the Phone feels
like everywhere, not just on the screen where you picked it.

**A theme-aware custom property layer.** `--phone-ink`,
`--phone-surface-subtle`, `--phone-border-subtle`, `--phone-base-1`/`-2`
(etc.) defined once at `.workshop-phone` (light defaults, identical to
the values that used to be hardcoded) and overridden at
`.workshop-phone[data-theme="dark"]`, reusing the existing paper/ink
token pairing inverted rather than inventing new hex values. Every
hardcoded occurrence across Wardrobe, Emotes, Browser, Settings, and the
rest swapped to the matching custom property — mechanical, one-for-one,
zero light-mode behaviour change.

**Brightness targets the screen, never the case.** `PhoneUI.js` gained a
`.workshop-phone-screen` wrapper around the status bar, header, content,
and home indicator, so `filter: brightness(var(--phone-brightness, 1))`
dims only the rendered display — the wood-and-brass case (border,
box-shadow) sits outside the wrapper and is untouched by it.

**Wallpaper and border pickers became real previews.** Each wallpaper
swatch renders the actual gradient formula via inline style, reading the
same live CSS custom properties the real content background resolves
against; border presets reuse the existing circular `.workshop-phone-swatch`
component Wardrobe's own outfit cards already established. A new Display
section (separate from Appearance) holds a Theme toggle (☀️ Light / 🌙
Dark) and Brightness (the existing `buildSliderRow()` helper, given a
0.3–1 range).

**Verified live**, not just read as correct (per `.claude/DEV_NOTES.md`):
confirmed `getComputedStyle` on three independent app screens (Wardrobe,
Emotes, Browser chrome) actually changed colour under dark theme, not
just Settings itself. One combined-selector query briefly returned
`rgb(0, 0, 0)` for a Wardrobe outfit card's own `color` — traced to the
card `<button>` element itself never having its own `color` rule (only
its child `.workshop-phone-outfit-name` span does) and the browser's own
UA default for buttons not inheriting colour the way a block element
would; confirmed the button has no direct text node of its own, so
nothing was actually rendering invisible — a test-script artifact, not a
real bug. Dragging the brightness slider confirmed the screen wrapper's
own computed `filter` changed while the case's border and box-shadow
stayed byte-for-byte identical. Selecting a wallpaper preset confirmed
its swatch's resolved gradient and the real content background's
resolved gradient were identical strings. Confirmed the PC Settings
app's own Phone tab is genuinely unchanged. `docs/PHONE.md` updated in
place, including correcting its own Wave 2 "Settings stays exactly as it
was" claim, now stale as of this phase. Debug hooks removed from
`main.js`, `.claude/launch.json` reverted to port 8000.

## Version 4, Phase 4 — Verification Tooling, and a Real Crouch Bug It Surfaced (v4.0.4)

**Goal:** `docs/ROADMAP_V4.md`'s own Phase 4 — not a Workshop feature but
an investigation into the unreliable-screenshot problem that had blocked
honest visual verification of several claims across two versions,
folding in a fresh field report Vi gave while scoping the phase: "the
player model doesn't lower when crouching... the players feet leave the
ground instead of the whole body moving down."

**Tooling investigation, re-confirmed rather than assumed stale.** A
fresh tab still reports `document.visibilityState: "hidden"`,
`hasFocus(): false`, and `computer{action:"screenshot"}` still times out
after 30s — the documented root cause holds. The roadmap's own named
escape hatch — a genuinely different, real browser surface — was
actually tested this time (a prior session could only speculate):
`mcp__claude-in-chrome__list_connected_browsers` returned empty, a real,
worth-recording negative rather than a permanent one. The working
pixel-readback technique (previously narrated prose only, from Version 3
Phase 2's shadow-bias work) is now a small, reusable `readRenderedFrame()`
helper in `.claude/DEV_NOTES.md`.

**Root-caused the crouch complaint with live numbers, not a guess.**
Built the actual rig via the page's own module graph and measured both
poses directly: standing ankle world Y `0.090`, crouched (old
`CROUCH_CLIP`) ankle world Y `0.306` — a real 0.216m float. Cause:
`CROUCH_CLIP`'s authored hip/knee bend is pure forward kinematics with
nothing correcting it, and with `torsoPivot` never translating (already
documented architecture), that shortfall shows up as the foot lifting,
not the body lowering. `FootIK.js` already had the right mechanism
(`solveTwoBoneIK()`/`applyLegIK()`, proven for idle-state terrain
correction) — it simply was never wired to the `"crouch"` movement
state.

**Fixed by extending the existing mechanism, not hand-tuning angles.** A
new `applyCrouchFootIK()` reuses `applyLegIK()`, targeting each ankle
back to the standing vertical span from its own hip, read live from the
rig's own segment lengths so it's correct for every body proportion the
Wardrobe can produce, not tuned for one default. Wired into
`PlayerAnimationSystem.js` beside the existing idle/terrain branch.
`CROUCH_CLIP`'s own authored leg angles are now inert (zeroed, with a
comment explaining why); torso lean and arm pose are untouched. **An
honest limit, found while implementing, not hidden:** the standing pose
already sits at ~99.99% of the leg's own maximum reach, leaving no real
slack to also bend the knee forward while holding the ankle at exactly
the same height — this fix targets exact height correctness (the
actually-reported bug) over a dramatic knee bend, which would need the
hip/torso to genuinely translate downward — the deeper, already-named
"foot IK's own job, a later milestone" limitation, squarely
`docs/ROADMAP_V4.md`'s own future "Rest of IK" phase.

**Re-verified live after the fix**, not assumed correct because the code
looked right: ankle Y gap closed from 0.216m to 0.0001m (matching the
two-bone solver's own built-in safety margin), confirmed against the
real live player rig (not just a synthetic default-body test), idle-state
terrain correction confirmed unaffected by a direct regression check.

**The original torso-visibility claim, finally pixel-verified.** A
combined-selector-style false start along the way: an exact-hex colour
match against the torso's real lit material read `0` even in an
obviously-correct control shot, nearly producing a false "invisible"
conclusion — traced to ordinary scene lighting shifting a lit material
away from its own flat base colour, not a real result. Fixed by swapping
the torso to a temporary unlit debug material and validating detection
against two independent controls (a camera clearly outside the
character; the same crouched eye position turned to look down at the
legs) that both correctly showed it. With the methodology validated:
**zero** matching pixels in a full 1280×720 frame from the actual
crouched first-person eye position looking straight ahead — the claim in
`docs/PLAYER.md`'s own "Known limitations" is confirmed against a real
rendered frame, not analysis alone. Along the way, this environment's
own renderer needed two one-time fixes never previously documented — a
0×0 canvas (`renderer.domElement` starts unsized) and a `null` camera
aspect (a degenerate projection matrix, silently producing a flat,
uniform frame with no error) — both now recorded in `.claude/DEV_NOTES.md`
so a future pixel-readback pass doesn't lose the same time rediscovering
them.

**Explicitly not touched**, per the investigation's own findings:
`docs/REFINEMENT.md`'s `CROUCH_HEIGHT_RATIO` tuning (subjective feel, no
tooling can judge it) and `docs/VISUAL_IDENTITY.md`'s shadow peter-panning
measurement (already tried pixel-readback, explicitly names a real
screenshot as the missing thing this session still can't produce). Both
already correctly describe themselves as deferred.

`docs/PLAYER.md` and `docs/ANIMATION.md` updated in place — including
correcting `docs/ANIMATION.md`'s own "IK is wired for one real gameplay
case" claim, now two.

## Version 4, Phase 5 — Lighting Fixtures for the Construction Library (v4.0.5)

**Goal:** `docs/ROADMAP_V4.md`'s own Phase 5 — build the `gardenLight`/
`streetLight`/`lantern`/`floodlight`/`campfire` pieces, "reserved as a
category since Version 1 — still zero real pieces behind any of them."

**That claim was wrong, and investigation found it before any code got
written.** All five already exist in `src/worldbuilder/ConstructionLibrary.js`
— real multi-part geometry, and a real `behaviours: [{ type: "lightSource" }]`
entry using the exact generic mechanism (`LightSourceBehaviour.js`,
`object3D.add(new THREE.PointLight(...))`) the original Utilities "Light"
piece already established. Verified live, not just read as correct:
spawning a real `campfire` instance (`worldObjectsStore.create()` +
`worldObjectsSystem.spawnInstance()`) put a genuine new `THREE.PointLight`
into the scene — point-light count went from 9 to 10, colour/intensity/
distance exactly matching the piece's own authored values — then removed
again, leaving no test artifact in save data. Mounting the real
`BuilderPhoneUI` in a detached container (the established technique)
confirmed all five appear as real, clickable entries in the actual
Construction Library list a player would use, not just present in the
data model.

**The roadmap's own guess at the mechanism was also wrong**, which is
likely why this read as unbuilt without checking: it named
`LightingSystem.js`'s `registerPracticalLight()` as the pattern to reuse.
That function is real, but scoped to a different, narrower job — letting
a *hardcoded, room-owned* light (the desk lamp, say) join the Workshop's
own physical light switch. Freely-placed Builder pieces have no single
switch to belong to; the already-existing "Light" Construction piece
never went through it either, and neither do these five.

**The actual gap was three stale doc claims, not missing code.**
`docs/WORLD.md` said, in three separate places, that these fixtures
"remain reserved, still unpopulated." The historical Version 2 Phase 9
("World Builder") account — accurately, for what it covers — explicitly
left Lighting out while filling in Nature and Paths for real, which is
presumably where the "still unpopulated" claim was copied from and never
revisited. But `docs/WORLDBUILDER.md` already correctly described the
end state ("Lighting — Garden Light, Street Light, and other fixtures,
each carrying the same Light Source behaviour the original ceiling Light
does"). Exactly which phase actually built them isn't reconstructable
from the available records — a genuine, undocumented addition, not a
mistake in any one phase's own account. Decided with Vi: fix the docs and
close honestly rather than build a duplicate of already-real work.

**Fixed**: `docs/WORLD.md`'s three passages corrected — the "Builder
Library" section's own historical account now explains what actually
happened rather than repeating the stale claim; the "Known
simplifications" bullet is struck through (kept, not deleted, since it
was genuinely true of that specific phase) with a note pointing here; the
"Future extension points" bullet is removed outright, since a still-open
todo list shouldn't list something already done. No `src/` changes — see
`CLAUDE.md`'s own "say what's already fine" rule.

## Version 4, Phase 6 — Being ↔ Resident Convergence, Investigation (v4.0.6)

**Goal:** `docs/ROADMAP_V4.md`'s own Phase 6 — determine concretely what
it would take for the resident (Bubble's own conversation memory,
traits, preferences, curiosity, world-awareness, expression sets) to
become a real `BeingLibrary` definition, rather than two structurally
separate systems that happen to share some vocabulary. Explicitly
investigation only, per the roadmap's own risk note — "prototype the
hardest architectural question first, before committing to an
implementation phase." No `src/` changes this phase; Phase 7 is where
anything actually gets built.

**Three parallel surveys, then direct re-verification of every
load-bearing claim** rather than trusting any one pass — `ConversationMemory.js`'s
own singleton comment, `BubblePhoneApp.js`'s hardcoded identity strings,
and `BeingInstanceStore.js`'s exact schema were all personally re-read,
not taken from survey summaries on faith.

**The Being side has no AI-shaped surface and no generic per-instance
data bag to grow one into.** A Being definition is an 18-field flat
schema with zero conversation/memory/personality fields — `beingType:
"resident"` and `interactionBehaviour: "talk"` both read like capability
stubs but have zero special-cased runtime behaviour anywhere, confirmed
by tracing `BeingController.js`'s actual per-frame loop. `BeingInstanceStore.js`'s
own instance schema is closed and hand-enumerated, no catch-all field —
`homeRadius` is the only existing precedent for "instance value diverges
from its definition," a single hardcoded field, not a generic mechanism.

**The AI side is already almost entirely profile-generic — the strongest
evidence this is tractable.** `ProviderRegistry`, `WorkshopFunctions`,
`PromptComposer`, and every `*Configuration.js` shape module operate on
an opaque `profile` object with zero hardcoded "Bubble" checks anywhere.
`getNearbyBeings` already queries a completely separate store from "the
resident" — the two are already architecturally parallel, not just
vocabulary-adjacent. The one real singleton is at the wiring layer in
`main.js`, not inside `src/ai/` itself.

**The resident side is more unevenly "already per-instance" than its own
docs claimed — verified module by module.** `ResidentTraits`/
`ResidentDials` (pure functions) and `ResidentContext` (explicitly takes
`(profile, deps)`) and `ResidentMovement` (self-contained state) are
genuinely per-instance today. `PlayerPatternMemory`/`ResidentPreferences`/
`ResidentCuriosity` assume a singleton in current wiring — zero-arg
constructors, no id parameter anywhere, despite `docs/RESIDENT.md`'s own
prior claim that they "already take a profile/instance as an argument."
`ConversationMemory` is the clearest case: its own header comment
self-admits it's "shared across every profile rather than segmented
per-resident" — a module the prior doc claim didn't even name. Five
concrete blockers to a second live resident today were traced to exact
file:line (`ResidentController` always reads `getActive()`, never a
specific profile id; `ResidentEntity.js` hardcodes both its entity name
and its conversation overlay id; the overlay registration and every
persistence key are singular; `ConversationMemory` needs real
restructuring).

**A real, independently-confirmed doc bug found along the way:**
`docs/PHONE.md` claimed Bubble's own Phone app was "already written
generically... not the name 'Bubble.'" True for the behavioural reads/
writes, but the app's own registry id, label, and heading fallback are
the literal string `"Bubble"`, three times — corrected in place.

**The concluded architecture, for Phase 7 to build:** a Being definition
gains a new `interactionBehaviour` value (`"aiResident"`) plus an
optional `residentProfileId` field, populated via `ResidentProfileStore`'s
own existing profile CRUD. `BeingInstanceStore`'s own schema stays
untouched deliberately; a new, parallel store keyed by Being instance id
becomes the home for what's currently `ConversationMemory`/
`ResidentPreferences`/`ResidentCuriosity`/`PlayerPatternMemory`/
`ResidentState`. The genuinely-per-instance resident modules
(`ResidentTraits`/`ResidentDials`/`ResidentContext`/`ResidentMovement`)
attach as-is; the singleton-in-wiring ones need re-keying, not logic
changes; `ConversationMemory` needs real restructuring. Interaction
reuses `BeingController.js`'s own already-generic `being:interact`
dispatch rather than trying to generalise `ResidentEntity.js`'s singular
wiring. **Decided with Vi:** a resident-capable Being keeps its own
player-designed visual identity — the resident's own translucent/glowing
visual treatment is deliberately not offered as an alternative
embodiment, so giving a Being real presence never means reskinning it as
the default resident. Bubble itself would become a real seeded
`BeingLibrary` definition under this shape, with a persistence migration
preserving every existing player's actual conversation history rather
than resetting it.

`docs/RESIDENT.md` and `docs/BEINGS.md` updated in place with the
corrected, concrete findings; `docs/PHONE.md`'s own overstatement fixed.

## Version 4, Phase 7 — Being ↔ Resident Convergence, Implementation (v4.0.7)

**Goal:** build exactly what Phase 6 concluded — a `BeingLibrary`
definition can gain a new `interactionBehaviour: "aiResident"` value plus
an optional `residentProfileId`, and Bubble herself migrates onto it as
the Workshop's own shipped example, not a special case the code still
checks for by name. The largest, riskiest phase in Version 4 so far, by
the roadmap's own risk note: every existing player's save had to
survive Bubble's migration with her real conversation history, mood, and
preferences intact, not reset.

**Schema, store, and movement, built as scoped.** `BeingBehaviours.js`
gained `"aiResident"`/`"residentTravel"`; `BeingLibrary` gained
`residentProfileId`. `BeingResidentStateStore.js` (new) wraps the exact
same, unmodified `ResidentState`/`ResidentPreferences`/
`PlayerPatternMemory`/`ResidentCuriosity`/`ConversationMemory` classes
per resident-capable Being instance id — none of those four files needed
internal changes, confirming Phase 6's own module-by-module survey.
`BeingController._updateResidentTravel()` reuses `ResidentMovement.js`
wholesale as a new movement style — **decided with Vi**, preserving
Bubble's exact existing travel feel rather than switching her to a
generic wander style. `ResidentController.js`/`ResidentEntity.js` are
deleted, not kept running in parallel.

**The blast radius was substantially larger than the approved plan's own
file list.** `ComputerSystem`, `AIApp.js`, `BubblePhoneApp.js`,
`ResidentService.js`, `DiagnosticsService.js`, and `WorkshopPages.js` all
consumed the old singleton wiring far more deeply than originally scoped
— discovered mid-implementation, not anticipated in planning. **Decided
with Vi, mid-phase:** keep Bubble's own dashboard/Phone app working
correctly, rather than also building a genuine multi-resident picker UI
this same phase — real, separate, well-scoped future work.

**A real migration-ordering bug, found by live verification, not
assumed away.** The v5→v6 migration's first pass set Bubble's own
`residentProfileId` to `null` and relied on `main.js`'s own boot-time
reconciliation to resolve it — but that reconciliation runs *after*
every provider's `load()`, including `BeingResidentStateStore.load()`'s
own resident-capability check, which needs a non-null
`residentProfileId` to hydrate a bundle at all. A hand-built v5 fixture
run through the real `migrateEnvelope()` caught it: her real old data
never made it into the new store, silently. Fixed by resolving
`residentProfileId` directly inside the migration itself, before any
provider loads — reverified against the same fixture, every field
(mood, position, conversation notes, preference counts) confirmed to
survive intact.

**A second real gap, found only because the docs were checked against
the running code, not the other way round.** `ResidentController.js` did
more than move Bubble — it also drove mood drift and a slow timer
sampling preferences, player behaviour patterns, and Being relationships.
The first implementation pass ported movement only; nothing caught that
mood and preferences had gone silently inert until a documentation sweep
cross-checked `docs/RESIDENT.md`'s own claims against what the shipped
code actually did. **Brought to Vi as a live finding, mid-close, not
silently patched or silently left broken** — chose "port it now, same
phase" over deferring it. `BeingController._updateResidentLife()`/
`_driftMood()`/`_samplePatterns()`, reconstructed from the documented
behaviour (the original method bodies didn't survive `ResidentController.js`'s
own deletion), restore it — verified live over roughly 53 simulated
minutes: mood genuinely cycled through nine states, weather/time-of-day/
location/activity preference bags all accumulated real counts, and a
Being placed deliberately nearby had its relationship affinity bumped
seventeen times.

**Honest, named simplifications, surfaced by the same docs sweep, not
fixed this phase.** Smaller than the mood/preference gap, and not raised
with Vi individually — tallied here for a future phase to pick up:

- The "quiet habit" (idle-location choice weighted by rain, golden hour,
  and the hourly clock chime) and personality trait/dial movement
  multipliers are both computed correctly but currently unread — nothing
  wires them into `_updateResidentTravel()`'s own location pick anymore.
- Load-time continuity repositioning is a silent no-op for a
  `residentTravel` Being — `BeingController._applyContinuity()`'s own
  generic reposition gets overwritten before the first frame by
  `ResidentMovement`'s own resume-from-persisted-position logic.
- Bubble's own soft "thinking" sound cue is unheard — its old watcher
  lived on the deleted `ResidentController.js`'s own per-frame loop.
- Mission Control's Status Card/Resident Health grid can show a
  different profile's name/model/provider than Bubble's own real one, if
  a player creates and activates a second profile for editing —
  Bubble-specific fields (mood, location) already read her own bundle
  correctly regardless.
- `ResidentRenderer.js`/the Expression System are genuinely orphaned —
  not a bug, a direct, already-approved consequence of Phase 6's own
  "Being keeps its own body" decision; kept as real, working, unmodified
  code rather than deleted, in case a future "unembodied resident" case
  wants it back.

**Verified live, beyond the migration fixture:** a second, genuinely
distinct resident-capable Being (own definition, own profile) held an
independent conversation with an independent memory bundle, confirmed
via direct object-identity comparison against Bubble's own; an ordinary
Being's toast interaction confirmed unaffected, side by side; a real
Ollama round-trip completed end to end through the actual conversation
UI.

`docs/RESIDENT.md`, `docs/BEINGS.md`, `docs/PERSISTENCE.md`, `docs/AI.md`,
`docs/PHONE.md`, `docs/ARCHITECTURE.md`, `docs/ATMOSPHERE.md`, and
`docs/AUDIO.md` all updated in place — every stale `ResidentController`/
`ResidentEntity` reference corrected or reframed as history, every gap
above named exactly where a reader would otherwise be misled.

## Version 4, Phase 7a — Closing Phase 7's Tallied Gaps (v4.0.7a)

**Goal:** close the five small, honestly-named-but-unfixed gaps Phase 7's
own documentation sweep left tallied for later, before starting Phase 8
("The Rest of IK") — none of them product decisions, each a mechanical
restoration of documented behaviour that depended on
`ResidentController.js`'s own deleted per-frame loop.

**Idle-location weighting, restored.** `BeingController._residentLocationWeights()`
reconstructs "A quiet habit" and "Resident awareness, extended" in one
place: rain/windy/golden-hour pull toward the window, storm/night pull
toward the Quiet Corner, a few minutes either side of the hour pull
toward the clock, an active project or the player standing at the
workbench/computer desk pull there, and a genuine accumulated favourite
gets a real boost. Verified live two ways: deterministic weight checks
across seven distinct environment/time/player states all matched
expectations exactly (including compound cases — a storm with real rain
correctly boosting both the window and the Quiet Corner at once), and a
4,000-trial statistical run through the real `_updateResidentTravel()`
pipeline during a mocked storm picked the Quiet Corner roughly twice as
often as any other location.

**Personality trait/dial movement multipliers, restored.**
`_updateResidentTravel()` now resolves `getPersonalityModifiers()` fresh
every frame and calls `ResidentMovement`'s own
`setRestDurationMultiplier()`/`setMovementSpeedMultiplier()`/
`setMotionDamping()`, plus scales the awareness radii by
`awarenessRadiusMultiplier`. Verified live: an extreme test profile
(Energy and Calmness both at their limits) produced exactly the values
`getPersonalityModifiers()`'s own trait-averaged-with-dials formula
predicts, genuinely different from a neutral profile's `1, 1, 1`.

**Load-time continuity for a `residentTravel` Being, restored.** A new
`_applyResidentContinuity()`, routed to from `_applyContinuity()`'s
existing loop, replaces the generic wander-reposition (which had been a
silent no-op for this movement style — whatever it set was immediately
overwritten by `ResidentMovement`'s own lazy construction). Gated on the
same 90-second `MIN_REST_SECONDS` threshold the original used; past it,
a weighted new location (reusing the gap above), arrived at directly via
`setDraggedPosition()`/`setDraggedLookAt()`. Verified live: below
threshold is a genuine no-op, above it produces a new named location with
`ResidentMovement`'s own internal state kept in sync.

**Bubble's own thinking-sound cue, restored.** `ResidentConversation.js`
gained an optional `audioSystem` dependency, called directly at the
existing `setThinking(true)` edge — no per-frame watcher needed at all,
unlike the original. Verified live: the call fires synchronously the
moment a message sends, with a real, non-null position matching the
resident's own actual location.

**A sixth fix, not a Phase 7 regression but found during the same sweep:**
Mission Control's Status Card and Resident Health grid could show a
different profile's name/model/provider than Bubble's own real one, if a
player created and activated a second profile purely for editing. A new
`residentDefinitionProfile()` helper in `AIApp.js` resolves Bubble's own
profile via her Being definition specifically for those two sections;
every editing section below still reads whichever profile is "active,"
unchanged. Verified live with a real second profile created and
activated.

**Left honestly unresolved:** the third of `_updateResidentTravel()`'s
own three originally-named simplifications — autonomous travel not
pausing during a conversation — since `BeingController` still has no
reach into the conversation overlay's own lifecycle; real, well-scoped
future work, not attempted this phase.

`docs/RESIDENT.md`, `docs/PERSISTENCE.md`, `docs/AUDIO.md`,
`docs/AI.md`, `docs/ATMOSPHERE.md`, and `docs/BEINGS.md` all updated in
...
## Version 4, Phase 9a — Constellations (v4.0.9a)

**Also fixed this session, before Phase 9 started, at Vi's own request:**
Bubble had quietly lost two things genuinely specific to residents when
Phase 7's "Being ↔ Resident Convergence" folded her own singular,
hand-wired interaction wiring into `BeingController`'s generic
`InteractableComponent` construction — reticle-gated interaction (not
distance-based; `InteractableComponent`'s own general-purpose
`requiresLookAt` flag, reduced radius to 1.2m) and a genuine
click-and-drag reposition mechanic (raw `mousedown`/`mouseup` canvas
listeners, deliberately not routed through the shared "interact" action,
since that pipeline can't distinguish a quick tap from the start of a
hold). Neither survived the Phase 7 rewrite as literal code — this isn't
a git repository, so nothing could be recovered directly — and both were
reconstructed from `docs/ROADMAP.md`'s own Version 2 "Living Refinement"
phase account, the only surviving description of the original design,
reusing `ResidentMovement.js`'s still-intact `setDraggedPosition()`/
`setDraggedLookAt()` primitives and `BuildModeSystem`'s own established
real-floor-raycast pair (`RoomLayoutSystem.getFloorMesh()` +
`TerrainSystem.mesh`). Scoped to any resident-capable Being
(`interactionBehaviour === "aiResident"`), not hardcoded to Bubble's own
id, matching the original account's own "not a resident-specific special
case" wording. Verified live: `requiresLookAt`/reduced radius confirmed
on Bubble's real spawned `InteractableComponent`; the drag mechanic
confirmed via real `MouseEvent`s dispatched on the canvas — gated
correctly behind pointer lock and no other open interaction, position
genuinely tracking a live floor raycast while held, idle travel resuming
cleanly on release; an ordinary Being (Person) confirmed completely
unaffected throughout. `docs/RESIDENT.md`/`docs/BEINGS.md` corrected.

**Goal:** the first of `docs/ROADMAP_V4.md`'s own Phase 9 ("Atmosphere,
Continued") four independently-shippable waves — Vi picked "a real
constellation catalogue" to start with; falling snow, lightning +
thunder, and seasonal effects remain for a future wave.
`docs/WORLD.md`'s own "Future extension points" scoped this precisely:
real right-ascension/declination positions for a modest named set,
slotted into the same simplified renderer, "without needing to change
how the rotation itself is applied" — not a latitude-corrected
planetarium.

**Six named constellations, real approximate coordinates, layered onto
the existing sky exactly as scoped.** A new `CONSTELLATIONS` catalogue in
`Astronomy.js` (Ursa Major, Cassiopeia, Cygnus, Orion, Scorpius, Crux —
36 stars, 30 asterism-line segments) and a new shared
`raDecToSpherePosition()`, resolved by two new `WorldEnvironmentSystem`
objects — a second `THREE.Points` (`constellationStars`, `size: 2.4`, a
deliberate step up from the background field's `1.6`, since real named
stars mostly are the brighter ones) and a `THREE.LineSegments`
(`constellationLines`, following `_buildShootingStar()`'s own precedent
for "a second small object") — both driven by the *exact same*
`stars.rotation.y`, `_starVisibility`/`clearFactor` opacity pipeline, and
per-frame camera recenter the background 320-star field already used,
untouched. Lines sit at half the points' own opacity ceiling, "a subtle
enhancement" per this file's own long-standing restraint principle, not
a bold overlay.

**An honest, stated-up-front simplification, not a bug found later:**
because the rotation this catalogue feeds only ever changes a star's
azimuth, never its altitude, a star's height is fixed forever by
`sin(declination)`. Scorpius's and Crux's real, far-southern declinations
(down to −63°) put every one of their stars permanently below the
horizon at this project's default 45°N observer config, at every hour;
Orion's belt sits almost exactly on the horizon plane, visibly
straddling it. Verified live and numerically, not assumed: recomputing
`raDecToSpherePosition()` for every catalogued star confirmed Ursa
Major/Cassiopeia/Cygnus all above Y=0, Scorpius/Crux all below, Orion
genuinely mixed — turning the design's own stated consequence into a
checked fact.

**Surfaced where players already look for astronomy detail, nothing
new invented:** the Computer's Settings app already had an Atmosphere-tab
"Stars" section (Visibility) — one more `infoRow` lists the six
catalogued names, with a hint noting it's a modest, real catalogue, not
the whole sky, and that a far-southern one like Crux won't appear at all
under this simplified model. No new interactive surface.

**Verified live, end to end, against the real running engine:** point/
segment counts matched the catalogue exactly (36/30); all three sky
objects' rotation stayed in lock-step across multiple simulated hours;
opacity tracked time-of-day (dark vs. midday) and weather (forcing a
storm dimmed both new objects proportionally, same as the existing star
field) correctly, with the line-to-star opacity ratio holding at exactly
0.5 throughout; both objects recentred on the camera every frame; the
real, mounted Settings app (via `createSettingsApp(computerSystem.deps)`,
the exact production dependency object `ComputerSystem` itself builds)
showed the new row with all six names; a pixel-readback of a real
rendered frame, camera aimed at Cassiopeia's own live rotated position
from an outdoor, night, clear-weather vantage point, showed a genuine
cluster of line-brightness pixels exactly where the math predicted —
confirming a visible pattern renders, not just that the numbers check
out. `docs/WORLD.md`'s own "Astronomy" section, two Known-simplifications
bullets, and one now-obsolete Future-extension-points bullet corrected;
`docs/ATMOSPHERE.md`'s matching stale passages corrected too.

## Version 4, Phase 9b — Falling Snow (v4.0.9b)

**Goal:** the second of Phase 9's four independently-shippable waves —
Vi picked falling snow after constellations. `docs/WORLD.md`'s own
"Known simplifications"/"Future extension points" named the exact gap:
"Snow has no dedicated visual... `WeatherProvider.js` maps [real snow
conditions] onto the closest rain-family state by intensity... rather
than inventing a snow visual this pass didn't ask for." This pass did.

**Two new weather states, tuned deliberately rather than copied from
rain.** `EnvironmentSystem.js` gained `lightSnow`/`heavySnow`, mirroring
the existing `lightRain`/`heavyRain` two-tier split (both in shape and
because the WMO snow codes being remapped already naturally split that
way). `lightDampening`/`fogDensity` sit slightly above the equivalent
rain tier (a snow-laden sky reads dimmer; falling snow scatters light
like a light mist); `precipitation` sits slightly below the matching
rain tier (flakes are bigger/slower/sparser per unit time than
raindrops — the same numeric value would over-render if copied
verbatim). `ambience: "wind"` reuses the existing, already-tuned
ambience id rather than adding new audio-synthesis scope
`AudioSystem._setAmbience()` (which only recognizes rain/storm/wind)
would need — genuinely honest, since hushed wind is a real
characteristic of snowfall anyway. `WeatherProvider.js`'s `WMO_CODE_MAP`
now resolves the real snow-family WMO codes (71/73/75/77/85/86) to these
new states instead of their old rain-family approximation — confirmed
against the live file that code 77 ("snow grains") was actually mapped
to `"drizzle"`, not `"lightRain"` as a first-pass summary assumed, and
corrected accordingly rather than trusting the paraphrase.

**A real, dedicated falling-particle visual, mirroring rain's own
architecture.** `WorldEnvironmentSystem.js`'s new `_buildSnow()` (called
from `init()` beside `_buildRain()`) builds a `THREE.Points` field —
flakes read as soft dots, not streaks, reusing `starSpriteTexture()`
directly rather than a new texture (`sizeAttenuation: true`, unlike the
star field's own `false`, since snow is real near-depth geometry that
should grow larger approaching the camera). Falls roughly an order of
magnitude slower than rain (`speed: 0.6-1.1` vs. rain's `5-8` — the
single biggest lever for reading as visually distinct, "not rain but
renamed"), drifts more with wind, and adds a small per-flake sideways
wobble rain doesn't have. Rain and snow share one `indoors`/
`precipOpacity` computation in `update()` — already mutually-exclusive
weather states — so a direct rain↔snow transition with no intermediate
clear state crossfades the two opacities smoothly rather than cutting;
verified live, switching `heavyRain` straight to `heavySnow` and
confirming the two opacities cross without ever both sitting near-full
simultaneously.

**A real bug corrected along the way, found while writing the adjacent
docstring:** `_buildRain()`'s own comment used to claim ordinary depth
testing alone was enough to keep rain out of view indoors — not true;
raindrops spawn in a box centred on the *camera*, so a good number end
up genuinely co-located with an indoor player rather than ever being
behind anything to begin with. The actual `indoors` check in `update()`
(now shared with snow) is what handles this; the docstring just never
said so. Fixed alongside the new snow work, not a separate pass.

**Decided with Vi: snow is reachable through Manual selection and Live
Weather, but deliberately absent from Workshop Dynamic mode's own
`TRANSITIONS` graph.** No season/temperature concept exists anywhere in
this system yet (`Astronomy.getSeason()` exists but stays unwired to
anything behavioural), so letting Dynamic mode reach snow on its own
would mean it could snow in a simulated summer, ungated by anything —
named as a stated simplification, with a still-unshipped future
seasonal-effects wave as the natural place to resolve it honestly.

**Verified live, end to end, against the real running engine:** manual
selection of both new states confirmed correct fog/cloud/tint values;
a direct rain→snow transition crossfaded cleanly; indoor exclusion
confirmed (opacity held at 0 indoors, rose once outdoors — the
player's own confirmed default spawn state); flake motion confirmed
genuinely changing frame to frame, falling ~7.9× slower than rain,
wrapping correctly at the top of its fall box; the WMO remapping and the
absence of both new states from `TRANSITIONS` confirmed directly against
the live source; the real, mounted Settings app showed both new options
in its Manual weather dropdown with zero code changes needed (already
generic); a differential pixel-readback (snow visible vs. snow opacity
forced to zero, identical camera/time/weather otherwise) isolated 1,371
snow-attributable bright pixels, not noise; rain's own behaviour
(opacity, fall speed, drop count, streak length) confirmed unchanged
throughout. `docs/WORLD.md` gained a new "Falling Snow" section and had
its stale "ten weather states"/"snow has no visual" passages corrected;
`docs/ATMOSPHERE.md`'s two matching stale mentions corrected too.

## Version 4, Phase 9c — Lightning + Thunder (v4.0.9c)

**Goal:** the third of Phase 9's four waves — a visible lightning bolt
and a real thunder sound, timed together with a delay proportional to
distance. `docs/WORLD.md`'s own brief named the exact gap:
`LightingSystem`'s existing storm flash only ever brightened the
hemisphere/ambient fill — no bolt, no thunder, no distance timing. The
existing flash cadence (occasional, 5-19 seconds apart during a storm)
stayed exactly as already tuned; this phase enriches what happens *at*
each existing trigger, not how often triggers happen.

**One real strike distance per flash, driving both halves.** At the
exact moment `LightingSystem` already sets its own flash to full
(`_lightningFlash = 1`), it now also rolls a random distance (200-4000m),
emits a new `lightning:flash` event immediately (light is ~instant), and
computes a real delay (`distance / 343`, the speed of sound) and volume
(closer = louder) for a genuine thunder sound — calling
`AudioSystem.playInteractionSound("thunder", {delay, intensity})`
directly, mirroring this same file's own existing `_checkClockChime()`
precedent, while the visual reaction (a different system) goes through
the EventBus, mirroring this same file's own `lightSwitch:flipped`
precedent — two established local patterns, reused for two different
reasons, not a new third one invented.

**Thunder is two layered noise bursts, reusing the existing synthesis
primitive rather than new low-level code.** `AudioSynth.js`'s shared
(unexported) `playFilteredNoiseBurst()` — already the basis for the
whole creak family — gained one new `startDelay` parameter, letting a
burst schedule itself real seconds into the future via the AudioContext's
own native clock rather than a JS timer (confirmed live: reliably
schedules 8+ seconds out, unlike `setTimeout`). New `playThunder()`
layers a sharp, brief "crack" with a longer, lower "rumble" starting just
after it — the way a real thunderclap's boom trails its own initial
snap — both louder at their peak than the deliberately-subtle creak
family, since thunder is meant to occasionally really register, though
still governed by the same volume chain as every other one-shot.

**A single jagged bolt, built once per flash, not travelling.**
`WorldEnvironmentSystem` listens for `lightning:flash` and draws a
`THREE.Line` at a random azimuth, descending from high overhead toward
the horizon — deliberately following the constellation-lines pattern
(build the shape once, recentre the whole object on the camera each
frame) rather than rain/snow's per-vertex rewrite, since a bolt's own
shape is static once triggered. Its own fade rate is the same number
`LightingSystem`'s own light-level pulse already decays at — duplicated
intentionally, not shared, so the two stay in sync without coupling the
systems — confirmed live, decaying in exact lockstep across ten frames
down to zero. No indoor exclusion needed for the bolt itself: it sits on
the same far sky dome as the stars, already correctly occluded by real
room/roof geometry via ordinary depth testing, confirmed via a
differential pixel-readback (0 bolt-attributable pixels aimed at it from
indoors, 390 from the identical aim outdoors).

**A real regression check, not assumed clean:** the existing hemisphere/
ambient-fill flash boost math was confirmed byte-for-byte unchanged, and
`_lightningTimer`'s own random cadence ranges were confirmed untouched —
this phase adds to the existing trigger, never touches how often it
fires.

**Verified live, end to end, against the real running engine:** the
event/bolt/decay chain (forcing a flash, confirming the event fired, the
bolt's opacity set to 1, and both systems' own decay values matching
exactly every frame thereafter); real Web Audio scheduling with a
monkey-patched `AudioBufferSourceNode.start()` spy confirming the actual
scheduled start times matched the hand-computed `now + delay` math to
the millisecond; distance→delay/intensity formulas checked against
200/2000/4000m samples; and the indoor/outdoor bolt-occlusion
differential above. `docs/WORLD.md` gained a new "Lightning + Thunder"
section and had its stale "light-only flash" Known-simplification and
now-resolved Future-extension-points bullet corrected;
`docs/ATMOSPHERE.md`'s matching stale mention corrected too.

**Also corrected this session, a versioning-scheme fix, not a feature:**
Phase 9's first two waves had been shipped as plain patch bumps
(`v4.0.9`, `v4.0.10`) rather than lettered sub-phases of Phase 9 itself.
Retroactively relabeled to `v4.0.9a` (constellations) and `v4.0.9b`
(falling snow) across `package.json`, both `docs/ROADMAP.md` phase
headers, both `docs/HISTORY.md` changelog entries, and every in-code
comment referencing the old "Wave 1"/"Wave 2" naming — confirmed by a
full repository sweep, not just the two files that started it. Going
forward, a multi-wave phase's waves stay lettered sub-versions of that
same phase number (`v4.0.9a`-`v4.0.9d` for however many Phase 9 waves
ship); Phase 10 will bump to a new minor version, `v4.1.0`, not another
`4.0.x` patch.

## Version 4, Phase 9d — Seasonal Vegetation Colour (v4.0.9d)

**Goal:** the last of Phase 9's four waves. `docs/ROADMAP_V4.md`'s own
brief: *"seasonal effects that actually change something visible or
behavioural... `Astronomy.getSeason()`, which already exists and is
currently unused for anything beyond a read-only label,"* with the
explicit guardrail that the right kind of change is one that *"change[s]
what already exists (vegetation colour, day length, resident
behaviour) rather than adding wholesale new geometry."* **Decided with
Vi:** vegetation colour only — resident behaviour stays deferred, and
day length turned out to already be real for free (see below), needing
a documentation correction rather than any code.

**Eight parts, one new flag, mirroring the exact wiring pattern this
codebase already established for "wind influencing vegetation."**
`ConstructionLibrary.js`'s Nature pieces (tree's two canopy spheres,
bush's three, the flower's own stem — not its bloom — grassPatch's
three) gained `seasonalFoliage: true` alongside their existing
`swaysInWind: true`. `ObjectCompiler.js` clones each tagged part's own
material before tagging it — the one genuinely non-obvious risk of this
whole wave, caught during design rather than after: `PlaceholderFactory
.js`'s `Materials.matte()` cache is keyed by hex string alone and shared
across every mesh in the world using that exact colour, so mutating it
in place would have retinted any other object sharing that hex by
coincidence, including a hypothetical player-authored one. Verified live:
the tinted mesh's material is a genuinely distinct instance from the
shared cache, and mutating it leaves the cached material untouched.

**`WorldObjectsSystem._applySeasonalTint()`** computes the real current
season (`Astronomy.getSeason()`, the identical call `SettingsApp.js`'s
own Season row already makes) and blends each part's authored base
colour toward a small fixed table (`SEASON_FOLIAGE_TINT`) via
`lerpColorHex()` — spring/summer untinted (the authored green already
reads as growing-season colour), autumn a warm amber (`#b3652c`, 0.6
strength), winter dulled and greyed (`#7d7362`, 0.6 strength). Applied
the instant a part spawns, and rechecked on an occasional 90-second
throttle — a season boundary is a once-per-~3-months event, not
something worth a per-frame cost.

**A real bug caught and fixed along the way, not assumed away:** the
existing wind-sway `update()` had a single early return keyed on
`_swayParts.length === 0` at the very top of the method — harmless while
`swaysInWind` and `seasonalFoliage` happened to overlap 100% on every
part authored so far, but it would have silently skipped seasonal
tinting entirely the moment a future part opted into one flag without
the other. Split into two independent guards rather than left coupled by
coincidence.

**Day length was already real, for free — a documentation correction,
not new work.** `solarPosition()`'s own `declination` term already
varies by day-of-year and feeds directly into `sunriseSunset()`'s own
horizon-crossing scan; summer days have always genuinely run longer
than winter ones since that formula shipped. Named plainly in
`docs/WORLD.md`'s own "Astronomy" section now, rather than left listed
as open work it no longer was.

**No indoor/outdoor distinction** — confirmed by design: unlike rain/
snow (physically falling particles a roof genuinely occludes), seasonal
colour is a static material property with no physical mechanism a roof
would block. Verified live: an indoor-placed bush tints identically to
an outdoor one.

**Verified live, end to end, against the real running engine:** spawned
real tree/bush/flower/grassPatch instances through the actual
`WorldObjectsSystem` pipeline, confirming immediate untinted colour
(today's real season, summer); all four seasons' tint math checked
against hand-computed `lerpColorHex()` values by temporarily forcing
each day-of-year; the 90-second throttle confirmed to hold before the
threshold and fire exactly at it; cleanup confirmed at both
`removeInstance()` and `_respawn()`'s own call sites, with no stale
entries left behind; the flower's own bloom confirmed to never carry the
seasonal flag and to stay pink in every season; the existing
`swaysInWind` animation confirmed completely unaffected by the new
split guard; and the real, mounted Settings app showing the corrected
Season hint text. `docs/ATMOSPHERE.md`'s "Season Foundations" section,
Known-simplifications bullet, and Future-extension-points bullet
corrected; `docs/WORLD.md` gained a new "Seasonal Vegetation Colour"
section and the day-length note above.

**Phase 9 ("Atmosphere, Continued") is now fully complete** — all four
waves (9a constellations, 9b falling snow, 9c lightning+thunder, 9d
seasonal vegetation colour) shipped. Phase 10 will bump to a new minor
...
## Version 4, Phase 10a — The Visual Upgrade, Foundation (v4.1.0a)

**Goal:** the first of five waves moving the Workshop's 3D content from
"a really good prototype" to a professional finish. This wave
deliberately redesigns *nothing* — it upgrades the shared layer every
object already comes through, so the other four waves build on a better
foundation instead of each re-solving the same two problems. Phase 10
bumps the minor version per the convention Phase 9c confirmed; the two
previously-drafted phases were renumbered to make room (Plugin SDK is
now Phase 11, Dormant Seams now Phase 12 — see `docs/ROADMAP_V4.md`).

**The investigation that set the scope.** The reported feeling ("the
basic 3d geometry is cool but it feels like a really good prototype")
traced to two shared, measurable causes rather than to any object's own
design. First: there was not one `normalMap`, `roughnessMap`, `aoMap` or
`bumpMap` anywhere in `src/` — grep across all 266 files, zero hits.
Every material was albedo-only, and a `MeshStandardMaterial` with only a
colour map cannot express surface micro-relief at all, which is why wood
read as wood-coloured paper. Second: every edge was a razor-sharp 90°,
because `PlaceholderFactory.box()` is raw `BoxGeometry` and it is behind
nearly every object here. Neither is a geometry-budget problem — which
is exactly why one shared wave could address both, for everything, at
once.

**Surface maps, derived from the albedo rather than drawn separately.**
`ProceduralTexture.js` gained `normalMapFromTexture()`,
`roughnessMapFromTexture()`, and the `surfaceSetFrom()` helper every
textured material now calls. They read `texture.image` — a
`THREE.CanvasTexture` keeps a live reference to the canvas it was built
from — and infer relief from luminance. That is the correct reading of
these particular textures rather than a shortcut: every generator in
that file already draws its detail *as* relief (grain lines are darker
because they are grooves, siding's board seams are shadow lines), so
luminance already is the height field. The practical consequence is that
no existing generator changed, no signature moved, and every furniture
file, builder piece and Being picks the improvement up untouched.

**Two real defects caught by verification, not by review.** Both were in
the first implementation, and both would have shipped as
expensive-but-invisible had this wave stopped at "the maps exist":

1. *The roughness maps were doing essentially nothing.* The first
   version mapped **absolute** luminance into the declared `[min, max]`
   band, which quietly made that band meaningless, because none of these
   generators use the full dynamic range — measured directly,
   `sidingTexture()` spans only 0.242–0.368, so a declared band of
   `[0.85, 1.0]` collapsed to an actual output of 0.886–0.906. A 2%
   variation, indistinguishable from a flat map, still costing a texture
   fetch per fragment. Fixed by normalising luminance across each
   texture's own observed range first; re-measured, every material's
   actual band now matches its declared band to within 0.001.
2. *Most normal strengths were far too weak, and one was already
   strong.* The initial values were guessed. Measured, the useful metric
   turned out to be *typical* (mean) normal tilt rather than peak — peak
   is one pixel in 65,000 and says almost nothing about how a surface
   reads — and wood and cork came out at ~1° typical, invisible, while
   brushed metal at a **lower** guessed strength came out at 15°,
   because its hard-edged 1px streaks produce far steeper luminance
   gradients than wood's soft 35%-alpha grain. Contrast and spatial
   frequency of the source drawing dominate; the strength number alone is
   not comparable between two generators. Retuned from measurement to
   typical tilts of wood 4.1°, siding 5.2°, cork 2.4°, metal 15.2°
   (unchanged), paper 0.8°.

**`bevelBox()`, built but deliberately not adopted.** A
`RoundedBoxGeometry`-backed helper whose outer extent stays exactly
`width × height × depth` — it rounds *inward* from the box's own faces,
verified to a delta of exactly 0 against a plain `box()` across five real
cases including an 8mm-thin clipboard page, where the radius clamp does
the work. It does **not** replace `box()`, and nothing adopts it this
wave, for two stated reasons: `RoundedBoxGeometry` generates different
UVs than `BoxGeometry` (a visible change on a textured surface, needing a
per-surface check), and silently rounding every box would have changed
all 56 construction pieces at once, unverified. Its measured triangle
cost is recorded in its own docstring — 108/300/588 at the three tiers
against a plain box's 12 — specifically so Wave 10c bevels selectively
rather than by default.

**A boot-time detail tier, and an honest limitation.** New
`src/utils/TextureQuality.js` decides how much detail to generate. It has
to be a boot-time decision: materials are built during system `init()`,
and a save — and therefore any real `SettingsStore` data — does not load
until the `engine:ready` event at the *end* of `engine.init()`. Rather
than invent a second, earlier `localStorage` read purely to beat that
ordering, it reuses the one heuristic the Workshop already trusts to make
exactly this call with no saved data available: `detectRecommendedPreset()`,
lifted out of `SettingsStore` as a standalone pure function with the
method delegating to it, so there is one implementation and two doors in.
The honest limitation, documented rather than hidden: a player who
manually overrides their graphics preset does not change their texture
tier until the next load. A live rebuild was considered and rejected —
`ObjectCompiler.js` deliberately *clones* its materials per instance, so
those clones would not pick the change up, and a half-applied rebuild is
a worse outcome than an honest boot-time decision.

**Verified live, against the real running engine**, per
`.claude/DEV_NOTES.md`'s established technique (a temporary
`window.__debugEngine`, removed before this close; a genuinely fresh tab
per check). The Workshop boots with 395 meshes and 134 unique materials
and zero console errors; the tier gate works end to end (this sandbox
reports `hardwareConcurrency === 4` and correctly auto-detects `low`,
with zero normal/roughness maps present in the real scene, exactly as
intended). The decisive check was pixel readback through the real
renderer rather than inspection of the texture objects: the same siding
material under the same raking light produced **2.02× the vertical
neighbour-to-neighbour contrast** with its maps attached (4.34 vs 2.14),
at essentially unchanged mean brightness (75.8 vs 77.1) — relief, not
merely darkening.

**One verification-methodology finding worth recording**, because it cost
real time and `.claude/DEV_NOTES.md` documents the opposite hazard:
cache-busted `import('...?v=' + Date.now())` creates a *parallel module
graph*, so a module's own un-busted internal imports resolve to a
different instance. Setting the texture tier on a `?v=`-imported
`TextureQuality` had no effect on the `ProceduralTexture` reading it,
producing a result that looked exactly like a code bug and was not. Where
DEV_NOTES warns that same-tab reloads leave modules *stale*, this is the
mirror image — for anything involving shared module state, use a
genuinely fresh tab with un-busted imports.

**Reviewed and found already fine:** the flat-colour materials
(`fabric`, `matte`, `plastic`, `rubber`, `ceramic`, `brass`, `emissive`)
were deliberately left untouched — they carry no `map`, so there is no
luminance detail to derive relief from, and a generated normal map would
be a flat sheet of `(0.5, 0.5, 1)` costing a texture fetch to do nothing.
Giving those surfaces real character means authoring detail they do not
have, which is Wave 10b/10c art work rather than something this wave
could honestly fake.

**Two stale docstrings fixed, found while reading.** `PartTypes.js`
claimed `THREE.CapsuleGeometry` "doesn't exist in the Three.js version
this project loads" — it landed in r142, this project pins r164, and
three files already use it (`BodyCompiler.js`, `ModelLoader.js`,
`BeingSpawnerSystem.js`); the comment predates a Three.js upgrade and was
never revisited. `PlaceholderFactory.js` promised a
`PlaceholderFactory.mesh("workbench", ...)` real-asset swap path that has
never existed in any form. Both rewritten to describe what is actually
true, with the old claim quoted so the correction is legible. Separately,
`docs/PRESENCE.md` was missing from `docs/ARCHITECTURE.md`'s docs index
entirely, and has been added.

**Deferred, with reasons:** blanket canvas-resolution increases across
every generator in `ProceduralTexture.js` — several tune their detail
density against a specific canvas size (`concreteTexture()`'s
`repeat.set(4, 4)`, `woodGrainTexture()`'s own documented tiling-seam
behaviour), so raising those is per-surface art work belonging with each
surface's own pass in Waves 10b/10c, not a global knob flipped here. The
one exception taken now is `metalBrushedTexture()`, which was the most
under-resolved surface in the Workshop at 128px and has no size-dependent
art tuning to disturb; it now follows the tier, with streak count, line
width and jitter all scaling linearly so the result is the same brushed
metal at higher fidelity rather than a different-looking material.

## Version 4, Phase 10b — The Visual Upgrade, Furniture and Room (v4.1.0b)

**Goal:** the second of Phase 10's five waves — the Workshop's own nine
furniture pieces and the room shell they stand in, now that Wave 10a's
shared foundation exists to build on.

**What this wave turned out to be, versus what it was planned to be.**
The Phase 10 plan named architectural trim — skirting, window reveals,
door casing — as this wave's single biggest win. **It was already
built.** `WorkshopRoom.js` has skirting on all four walls, genuinely
hollow window frames with real sills, and a proper door casing with
jambs and header, all added by the earlier "Workshop Interior" phase and
all still correct. Reported plainly rather than quietly re-scoped, per
`CLAUDE.md`'s own rule 6: the plan was written from the roadmap and the
furniture files, before this wave's own read of the room, and the room
was further along than the plan assumed.

Reading the room instead surfaced three gaps that were larger than the
one that had been planned for:

**1. The largest surfaces in the Workshop had no texture at all.** The
interior walls and the ceiling were `Materials.matte()` — a completely
flat colour. Measured through the real renderer, a matte wall's
neighbour-to-neighbour surface detail reads **0.011**, which is to say
mathematically featureless: every carefully grained wooden object in the
room was being seen against a void. New `plasterTexture()` +
`Materials.plaster()` give both a real painted-plaster surface at
unchanged colours, measuring **1.798** under the same light with mean
brightness preserved (139.2 → 138.3) — relief, not tinting.

**2. Three surfaces built their own materials and so were skipped by
Wave 10a entirely.** `deskTopMaterial()`, `benchTopMaterial()` and the
floor each hand-rolled a `new THREE.MeshStandardMaterial({ map: ... })`
rather than going through the `Materials` factory. That was harmless for
years and stopped being harmless the moment Wave 10a added derived
surface maps *inside* that factory — at which point the desk top and the
bench top, the two wood surfaces a player looks at longest and closest
in the entire Workshop, had quietly become the only grained wood in it
without any relief, and the floor the only concrete. All three now use
`surfaceSetFrom()`; the floor moved onto a new `Materials.concrete()`,
removing a second way of doing the same thing.

**3. Every edge was still sharp.** 64 meshes across the nine furniture
pieces and the room's two straight skirting runs now use Wave 10a's
`bevelBox()` — chosen per object rather than globally: hero surfaces
(desk top, bench top), moulded plastic (monitor bezel, keyboard, mouse,
storage bins), case furniture (wardrobe body, cornice, plinth, cabinet
carcass, shelf boards), and most emphatically upholstery, where a
cushion having no sharp edges at all in reality makes it the strongest
case in the Workshop. Deliberately *not* applied to small cylindrical
details, thin decorative props, or anything where the chamfer would be
below the threshold of visibility while still costing triangles.

**A UV claim from Wave 10a, corrected.** That wave's own `bevelBox()`
docstring asserted `RoundedBoxGeometry` "generates its own UVs from a
subdivided cube, which do not match `BoxGeometry`'s," and named that as
the reason every textured surface needed an individual check before
adopting bevels. Measured directly this wave on a real bench-top size
(1.8 × 0.08 × 0.8): **both lay out per-face UV islands spanning 0–1**,
and the rounded version's flat top face spans u 0.005–0.995, v
0.012–0.988 — inset by exactly the bevel's own proportion of the face,
which is correct rather than wrong, and far below visibility. The
blocker wasn't one. Corrected in place rather than left standing.

**A real tuning error caught by measurement, not review.** Plaster's
normal strength was first set to 3, which measured at **~4.7° typical
tilt — stronger than the wood furniture standing in front of it**,
exactly backwards for a backdrop. Retuned to 1.5 (~2.4°), and concrete
raised from 4 to 14 (0.75° → ~2.0°, since `concreteTexture()`'s own
1st–99th-percentile contrast is only 0.027 and needed far more scaling
than a guess suggested). The resulting hierarchy is deliberate and
verified: **wood 3.81° > plaster 2.41° > concrete 2.02°** — the room
reads as real surface while staying subordinate to what stands in it.
`plasterTexture()`'s own comment was also corrected: it claimed the
texture's contrast was "a few percent," where it actually measures 0.129
across the 1st–99th percentile. The reason it doesn't produce visible
seams between differently-sized wall segments is that it's *formless*
(scale-invariant speckle), not that it's faint — a different fact than
the one originally written down.

**Cost, measured.** Furniture across all nine pieces: 6,040 triangles
before bevelling, 11,992 / 23,896 / 41,752 at the low/medium/high tiers.
The full scene runs 395 meshes and 38,354 triangles at the tier this
environment auto-detects. Trivial for any GPU this project targets.

**Verified live:** zero console errors on a clean boot; 64 bevelled
meshes present in the real scene; textured materials up from 260 to 270
(the walls and ceiling that previously had none); and every declared
`footprint` and `focusPoseLocal` confirmed byte-identical, which is the
invariant this wave promised — furniture collision reads declared
footprints rather than mesh bounds, and `bevelBox()` preserves its
envelope exactly (verified to a delta of 0 in Wave 10a), so neither
collision nor any camera pose moved.

**Deferred, with reasons:** the flat-colour materials (`fabric`,
`matte`, `plastic`, `rubber`, `ceramic`, `brass`) still carry no texture
and so still gain no relief — giving them character means authoring
detail they don't have, and doing it well is a larger art pass than this
wave's remit. Upholstery is the most visible case and the natural first
candidate if a later wave revisits it. Per-segment UV correction for
wall segments was also considered and rejected: it would mean rebuilding
how every wall segment is generated, and the formless-texture approach
makes it unnecessary.

## Version 4, Phase 10c — The Visual Upgrade, Construction Library (v4.1.0c)

**Goal:** the third of Phase 10's five waves — all 56 Construction
Library pieces and the default blueprints built from them.

**Vi's decision that shaped this wave, recorded because it changes what
was permissible:** a placed builder object stores no geometry at all,
only `{definitionId, position, rotation, scale}`, resolving its shape
from `ConstructionLibrary.js` at load time — so editing a piece
retroactively reshapes every already-placed copy, and changing its
*dimensions* silently opens gaps in finished structures whose absolute
positions were frozen at placement. Vi holds the only save, the project
is still in development, and chose to allow dimension changes and start
a fresh save after this pass. Without that, this wave would have been
held to strict envelope preservation and two of the fixes below could
not have been made.

**Every piece was flat, untextured colour — and the seam to fix it
already existed.** `ObjectCompiler.js` understood exactly one
`materialType` value (`"glass"`) and defaulted everything else to
`matte`, so a timber table and a concrete wall were the same surface in
two tints. Rather than invent a mechanism, that field was widened to
resolve **any** key on the shared `Materials` factory — deliberately the
same shape `BodyCompiler.js` already established for Being parts. 98 of
134 parts now declare what they are made of; **110 parts render
textured against 0 before**. The field names still differ between the
two compilers (`materialType` here, `material` there) and that is left
alone on purpose: both are persisted in user data, so unifying them
would mean a migration for cosmetic consistency and no functional gain.

**Bevels, and the one genuine subtlety in this wave.** Every other
geometry in `ObjectCompiler.js` is built at unit size and sized through
`mesh.scale`, which is what lets a hundred placed Walls share one
geometry. A rounded box cannot work that way — `RoundedBoxGeometry`
takes a single corner radius, so scaling a unit one by `[2, 2.5, 0.2]`
stretches that radius per-axis into a two-metre sweep along the top edge
and a barely-visible one along the side. Bevelled parts therefore bake
their real dimensions into geometry and keep `mesh.scale` at identity,
with the cache keyed on those dimensions — **sharing is preserved, not
sacrificed**, since a piece's dimensions are fixed by its definition
(verified: two separately compiled Tables return the identical geometry
instance). 16 parts opted in, chosen for proximity rather than
importance: workshop furniture, doors and leaves, the carried book, the
sign board, the switch plate. Structural pieces are deliberately
excluded — they are placed in the largest numbers and a chamfer on a
wall seen across a room buys nothing for 25× the triangles.

**Three real bugs found by reading, none of them cosmetic:**

1. **The Lantern was broken, not merely plain, and had been since Phase
   5.** Both its parts sat at the identical position `[0, 0, 0]` with
   the "glass" (0.12³) entirely *inside* the opaque "body" (0.16 ×
   0.22 × 0.16) — invisible from every angle, so the piece rendered as a
   plain dark box emitting light from nowhere. The glass part also never
   carried `materialType: "glass"`, so it would have been opaque even if
   visible. Rebuilt as a lantern actually is: base and cap with real
   glazing between four corner posts, same envelope, same id, same light
   behaviour.
2. **`doorLeafLeft` and `doorLeafRight` were missing from
   `CONSTRUCTION_GROUPS` entirely**, and had been since Version 4 Phase
   2 added them. The file's own comment anticipates exactly this ("a
   future addition that forgets to update this") and the fallback did
   its job — but the two most realistic door pieces in the library were
   quietly filed under "Other", away from every other Opening. Every
   piece now resolves to a real group; verified, not assumed.
3. **`CONSTRUCTION_GROUP_ORDER` listed a `"Structures"` group no piece
   has ever mapped to.** Invisible rather than broken (an empty group
   renders as nothing), but a heading waiting for content that was never
   coming. Removed.

**Two proportion fixes the fresh-save decision made possible:**

- **The Stairs were five floating slabs.** Each step was a board exactly
  one rise tall, hanging at its own tread height with nothing beneath
  it — walk beside a placed staircase and you saw daylight between the
  steps. Each step now runs from the ground to its own tread, the
  "wedding cake" construction a real flight has. The outer envelope is
  unchanged; only the void underneath is filled.
- **Neither window had an actual sill.** The part named `sill` on both
  pieces is really the apron — the wall *below* the opening. There was
  no projecting ledge anywhere, which is a window's most recognisable
  detail and the thing that makes one read as built into a wall rather
  than cut out of it. Added to both, oversailing the wall face on each
  side as a real sill does.

**Reviewed and found already fine:** 21 parts remain `matte`, and every
one is a missing *texture* rather than a missed assignment — all foliage
(no leaf or bark texture exists, and the wood grain that does exist
would read worse on a canopy than a clean flat colour), the light
fixtures and campfire flame (surface relief fights the read of something
emitting light), and soil/dirt (no earth texture). Recorded in the file
rather than left to look like an oversight. Player-authored Builder
objects are also confirmed completely unaffected: no `materialType`
means the same `matte` and the same unit-scaled sizing they always had.

**A count this wave corrected:** the library has **56** pieces, not the
55 that Waves 10a and 10b both stated. The earlier figure came from a
grep whose pattern missed one differently-indented declaration;
`CONSTRUCTION_PIECES.length` was measured directly this time. Corrected
in `PlaceholderFactory.js`, `ConstructionLibrary.js`, `ObjectCompiler.js`,
`docs/HISTORY.md`, `docs/ROADMAP_V4.md` and this file's own 10a account.

**Verified live:** all 56 pieces compile with zero errors; every
bevelled part's geometry matches its declared scale to within 1e-6 (the
load-bearing invariant for baking dimensions in); no part is double-
scaled; geometry sharing across instances confirmed intact; every piece
resolves to a group present in `GROUP_ORDER`; default blueprints
reference no missing pieces; and a clean boot with zero console errors.
Total geometry for the entire library is 13,052 triangles.

## Version 4, Phase 10d — The Visual Upgrade, Player and Beings (v4.1.0d)

**Goal:** the fourth of Phase 10's five waves — the player rig and the
Workshop's three starter Beings.

**The plan for this wave was wrong, and reading the code is what caught
it.** Phase 10's own outline called for "shaped, tapered, capsule-based
silhouettes replacing plain boxes" for *both* the player and the Beings.
That is right for one of them and actively wrong for the other:

- **The player's boxiness is the brief.** `PlayerCharacter.js`'s own
  opening comment quotes it directly — "think along the lines of
  Minecraft... simple geometry is preferred because it allows complete
  customisation." Rounding the player into organic capsules would have
  overturned a documented design decision in the name of polish, which is
  precisely the trade `docs/HANDBOOK.md` warns against.
- **And the texture pipeline depends on it.** A part's painted texture is
  a single 64×64 canvas applied as `map`. `BoxGeometry` gives every face
  its own 0..1 UV island so that image reads correctly on each face; a
  capsule's UV is a cylindrical wrap, which would smear every existing
  painted skin and outfit around the limb.

So the player got craftsmanship *within* its vocabulary instead — the
same thing Waves 10b/10c did for furniture. `boxMesh()` now builds
through the shared `bevelBox()` helper with a radius proportional to each
part's own smallest dimension (a hand and a torso want visibly different
softness). A bevelled box is still unmistakably a box; it just catches a
highlight along every edge. **Wave 10b's measurement is what made this
safe**: `RoundedBoxGeometry` lays out per-face UV islands spanning 0..1
exactly as `BoxGeometry` does, so every existing painted texture still
maps correctly — verified again here (torso UV range 0..1, mesh size
exactly the declared 0.52 × 0.72 × 0.32).

**The Beings are where real shape work belonged**, because a Being is
authored through the Being Creator, which has offered capsules and
cylinders since Version 3 Phase 10c precisely so a creature needn't be a
stack of cubes — and yet not one of the Workshop's own starter Beings
had ever used anything but a box and the default `matte`. The shipped
examples were quietly demonstrating less than the tool can do. Person's
four limb pairs became capsules; fabric materials went on across all
three Beings.

**A wrong docstring caught by measuring, with real consequences.**
`BodyCompiler.js` claimed its unit capsule was sized "so the whole shape
roughly fills the same unit bounding box every other primitive here
does, so `part.scale` behaves consistently no matter which shape is
picked." Measured directly: the capsule is **0.6 × 1.0 × 0.6** where
box, sphere and cylinder are all exactly 1 × 1 × 1. So swapping a part
to a capsule at the same scale silently makes it **40% thinner** — the
opposite of the promised consistency, and it would have quietly slimmed
every limb had this shipped unmeasured. Person's capsule limbs divide X
and Z by 0.6 to compensate, verified restoring the exact original
widths (0.16, 0.14, 0.2, 0.17) with lengths untouched. The geometry
itself was deliberately *not* "fixed" to fill the unit box: every
capsule a player has already authored is sized against these numbers,
and changing them would resize all of them at once. The honest fix is
the accurate comment.

**Something tried, measured, and reverted:** Cat and Dog's bodies were
switched to capsules and switched back. Their long axis runs along Z, and
a capsule's length only ever runs along Y — `scale` can't lie one on its
side (scaling Z widens the round section rather than lengthening it), and
`rotation` in `BodyCompiler` applies to the *pivot*, which for
`cat-body`/`dog-body` is the rigged `torso` joint the quadruped clips are
authored against. A capsule barrel would need either an unrigged part or
a mesh-only rotation this compiler doesn't offer. Recorded in both files
rather than left as an unexplained box.

**Two more stale docstrings fixed in `PlayerCharacter.js`.** Its opening
paragraph claimed body sections were "unit-sized box(es)... never unique
geometry per proportion, same as `ObjectCompiler.js`" — that was never
true of this file, which has always built geometry at each part's real
dimensions, and it was describing a different file's approach as though
it were this one's. `applyPose()` also carried two stacked JSDoc blocks
saying largely the same thing, the result of an earlier edit adding a
second rather than extending the first; merged.

**Bubble was deliberately left completely untouched** — she is a
`residentEmbodiment` body source, not a compiled body, and Version 4
Phase 7b restored that embodiment specifically after an earlier phase
wrongly folded her into the generic player-designed-identity rule.
Nothing about a visual-polish wave justifies revisiting that.

**Verified live, against the real animation stack rather than by
inspection:** all 14 built-in clips driven through the real `ClipPlayer`/
`advanceFrame`/`computeBlendedPose` path and applied to the rebuilt rig
across 420 pose applications, zero errors; `FootIK.applyCrouchFootIK()`
exercised on the real rig; `applyPoseToMappedSkeleton()` exercised
against both Person's biped skeleton and Cat's quadruped one. Every
joint's world position confirmed unchanged on all three Beings, and all
